...
async function resolveWorkingSite() {
    ...
    if (!candidates.length) throw new Error('91jav 未配置可用域名')

    let lastErr = ''
    for (const site of candidates) {
        const testUrl = `${site}/index/getMvStyle/order/count`
        try {
            const { data } = await $fetch.get(testUrl, {
                headers: {
                    'User-Agent': UA,
                    Referer: site + '/',
                    Origin: site,
            ...
            if (isParkedOrRedirectPage(html)) continue
            const $ = cheerio.load(html)
            if ($('.pb-3.pb-e-lg-40 .col-6.col-sm-4.col-lg-3').length <= 0) continue
            appConfig.site = site
            return site
        } catch (e) {
    ...
    throw new Error(`91jav 所有候选域名均不可用: ${candidates.join(', ')}${lastErr ? ` (${lastErr})` : ''}`)
}
...
async function getWebsiteInfo() {
  ...
  return {
    name: String(cfg.title || '91jav (無海報)'),
