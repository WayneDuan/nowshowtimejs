...
function buildPageUrl(url, page) {
    ...
    if (!finalUrl) finalUrl = `${appConfig.site}/catalog`
    if ((Number(page) || 1) <= 1) return finalUrl
    return finalUrl + (finalUrl.includes('?') ? '&' : '?') + `page=${page}`
}
function parseCardsFromDoc($) {
    const cards = []
    const seen = new Set()
    const nodes = $('.thumbnail, .video-item, .videos .item, .video-list .item')
    nodes.each((_, element) => {
        const root = $(element)
        const titleAnchor = root.find('.video-title a, .title a, a[title]').first()
        const title = String(titleAnchor.text() || titleAnchor.attr('title') || '').trim()
        if (!title || title.includes('[廣告]')) return
        const href = String(titleAnchor.attr('href') || root.find('a[href*="/video/"]').first().attr('href') || '').trim()
        if (!href || seen.has(href)) return
        seen.add(href)

        const subTitle = String(root.find('.video-tag, .tag, .label').first().text() || '').trim()
        const duration = String(root.find('.video-duration, .duration').first().text() || '').trim()
        const pubdate = String(root.find('.video-date, .date, .time').first().text() || '').trim()

        let cover = ''
        const preview = root.find('.preview-video').first()
        const style = String(preview.attr('style') || '')
        const m = style.match(/url\(['"]?(.*?)['"]?\)/i)
        if (m && m[1]) cover = absUrl(m[1])
        if (!cover) {
            const img = root.find('img').first()
            cover = absUrl(img.attr('data-src') || img.attr('data-original') || img.attr('src') || img.attr('poster') || '')
        }
...
async function getTabs() {
    ...
    const classAnchors = $(
        '.swiper-wrapper > .swiper-slide a.btn-categories, a.btn-categories, a[href*="/catalog/"], a[href*="/category/"]'
    )
    classAnchors.each((_, e) => {
        const node = $(e)
        const hrefRaw = String(node.attr('href') || '').trim()
        if (!hrefRaw || hrefRaw === '#' || hrefRaw.startsWith('javascript:')) return
        const href = absUrl(hrefRaw)
        if (!href || seenHref.has(href)) return
        seenHref.add(href)

        const name = String(node.find('.btn-categories__title').text() || node.text() || '').trim()
        const info = String(node.find('.btn-categories__info').text() || '').trim().split(' ')[0]
        const isIgnore = isIgnoreClassName(name)
        if (isIgnore || !name) return
